...
User.findUserByNickname = (UName, result) => {
  connection.query(`SELECT * FROM user WHERE UName = ${UName}`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
//-------------------------------
  ...
  User.updateUserById = (UID, user, result) => {
    connection.query(
